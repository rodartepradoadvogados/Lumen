...
| Aviso de figurinha (WhatsApp) fora do webhook | `AVISO_FIGURINHA_INTERNA_SECRET` | Autoriza `POST /api/whatsapp/aviso-figurinha/processar` — a rota interna que espera os 15s e deixa a Ana avisar quando chega só uma figurinha (`lib/avisoFigurinha.ts`). Sem esta variável, o disparo imediato não tenta chamar a rota (fail-closed) e o aviso só sai pela rede de segurança do cron `avisos-de-figurinha` (a cada 5 minutos) — mais lento, nunca ausente. |
