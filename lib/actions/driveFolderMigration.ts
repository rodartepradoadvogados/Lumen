...
async function processarEntidade(
  ...
  try {
    ...
    if (!info) {
      return { ...base, action: "PASTA_INEXISTENTE", detail: "A pasta não foi encontrada no Drive (id não existe mais) — pode ter sido apagada, ou movida manualmente para a Lixeira. Nenhuma ação foi tomada." };
    }
...
export async function migrarPastasLegadasDoDrive(simulacao: boolean): Promise<DriveFolderMigrationResult | { error: string }> {
  ...
  for (const c of cases) {
    ...
    entries.push(await processarEntidade("PROCESSO", c.id, c.title, c.driveFolderId, rootProcessos, officeId, simulacao));
  }
  for (const a of attendances) {
    if (!a.driveFolderId) continue;
    entries.push(await processarEntidade("ATENDIMENTO", a.id, a.subject, a.driveFolderId, rootAtendimentos, officeId, simulacao));
  }
