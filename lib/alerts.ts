...
export type AlertItem = {
  id: string;
  kind: "PRAZO_VENCIDO" | "PRAZO_PROXIMO" | "PUBLICACAO_NAO_LIDA" | "CONTA_PAGAR_VENCIDA" | "CONTA_RECEBER_VENCIDA" | "MENCAO";
  title: string;
  subtitle?: string;
  date: Date;
  href: string;
...
export async function getAlerts(): Promise<AlertItem[]> {
  ...
  const [overdueTasks, upcomingTasks, unreadPubs, overduePayables, overdueReceivables, unreadMentions] =
    await Promise.all([
      ...
      prisma.payable.findMany({ where: { status: { in: ["PENDENTE", "ATRASADO"] }, dueDate: { lt: now } } }),
      prisma.receivable.findMany({ where: { status: { in: ["PENDENTE", "ATRASADO"] }, dueDate: { lt: now } } }),
