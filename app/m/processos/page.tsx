...
export default async function MobileProcessos({ searchParams }: { searchParams: { q?: string } }) {
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      <div>
        <h1 className="font-serif text-xl font-bold text-navy-900">Processos ativos</h1>
        <p className="text-sm text-navy-800/50">{cases.length} registro(s)</p>
      </div>
      ...
      <form className="flex gap-2">
        ...
        <button type="submit" className="bg-navy-900 text-white text-sm font-semibold rounded-lg px-4 py-2">
          Buscar
        </button>
      </form>
          ...
          <div className="divide-y divide-navy-800/5">
            {cases.map((c) => (
              <Link key={c.id} href={`/m/processos/${c.id}`} className="flex items-center gap-3 px-4 py-3.5">
                <span className="h-10 w-10 rounded-lg bg-navy-900/5 text-navy-800 flex items-center justify-center shrink-0">
                  <Scale size={18} />
                </span>
