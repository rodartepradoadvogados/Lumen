...
export default async function MobileCaseDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      <Link href="/m/processos" className="inline-flex items-center gap-1 text-xs font-semibold text-navy-800/50">
        <ArrowLeft size={13} /> Processos
      </Link>

      <div>
        <h1 className="font-serif text-lg font-bold text-navy-900 leading-tight">{c.title}</h1>
      ...
      <Card className="p-4 space-y-2.5">
        ...
        {c.description && (
          <div className="pt-1">
            <p className="text-xs font-semibold text-navy-800/50 uppercase tracking-wide mb-1">Descrição</p>
            <p className="text-sm text-navy-800 whitespace-pre-wrap">{c.description}</p>
          </div>
        )}
      </Card>
      ...
      <Card>
        <div className="px-4 py-3 border-b border-navy-800/8">
          <h2 className="font-serif font-bold text-navy-900 text-sm">Próximas tarefas</h2>
        </div>
        {c.tasks.length === 0 ? (
            ...
            {c.tasks.map((t) => (
              <div key={t.id} className="px-4 py-3">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  ...
                  <span className="text-xs font-semibold text-navy-800/55">{formatDate(t.dueDate)}</span>
                  {t.dueTime && <span className="text-xs text-navy-800/45">{t.dueTime}</span>}
                </div>
      ...
      <Card className="p-4 space-y-4">
        <h2 className="font-serif font-bold text-navy-900 text-sm">Comentários</h2>
        {c.comments.length === 0 ? (
...
function Field({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="flex justify-between gap-3 text-sm border-b border-navy-800/5 pb-2 last:border-0 last:pb-0">
      <span className="text-navy-800/50 shrink-0">{label}</span>
