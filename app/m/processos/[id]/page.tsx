...
export default async function MobileCaseDetail({
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      ...
      {anexosFalhos > 0 && (
        <div className=" border border-urgente bg-urgente-bg rounded-[2px] px-3 py-2.5 text-corpo text-urgente">
          {anexosFalhos === 1
      ...
      <div>
        ...
        <p className="flex flex-wrap items-center text-corpo text-tx-2 mt-1">
          {c.processNumber && (
            <>
              <CopyButton
      ...
      {tab === "visao-geral" && (
        <Card className="p-4 space-y-2.5">
          ...
          <div className="flex items-center justify-between -mt-1 -mr-1">
            <h4 className="text-corpo font-semibold text-tx-2 uppercase tracking-wide">Dados do Processo</h4>
            <EditCaseModal
              caseData={{
                id: c.id,
                title: c.title,
          ...
          <div className="pt-1">
            <p className="text-corpo font-semibold text-tx-2 uppercase tracking-wide mb-1">Descrição</p>
      ...
      {tab === "visao-geral" && (c.materias.length > 0 || c.assuntos.length > 0 || c.distributedAt) && (
        <Card className="p-4 space-y-2.5">
          <h4 className="text-corpo font-semibold text-tx-2 uppercase tracking-wide">Classificação</h4>
          {c.materias.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {c.materias.map((m) => (
                <span key={m} className="text-corpo font-semibold px-2.5 py-1 rounded-full bg-marca-bg text-marca-tx">
                  {m}
                </span>
              ))}
            </div>
      ...
      {tab === "visao-geral" && caseLinks.length > 0 && (
        <Card className="p-4 space-y-2">
          <h4 className="text-corpo font-semibold text-tx-2 uppercase tracking-wide">Processos vinculados</h4>
                ...
                <div key={t.id} className="flex items-start gap-2.5 px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      <span className="text-corpo font-semibold text-tx-2">{formatCalendarDate(t.dueDate)}</span>
                      {t.dueTime && <span className="text-corpo text-tx-2">{t.dueTime}</span>}
                    </div>
            ...
            <div className="space-y-3">
              {comments.map((cm) => {
                ...
                <div key={cm.id} className="flex gap-2.5">
                  <div className="h-8 w-8 rounded-full bg-grafite-700 text-rail-marca flex items-center justify-center text-corpo font-bold shrink-0">
