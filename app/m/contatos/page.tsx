...
export default async function MobileContatos({ searchParams }: { searchParams: { tab?: string; q?: string } }) {
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      <Link href="/m/mais" className="inline-flex items-center gap-1 text-corpo font-semibold text-tx-2">
      ...
      <div className="flex gap-1.5 overflow-x-auto scrollbar-thin -mx-1 px-1">
        {TABS.map((t) => (
          <Link
            ...
            className={`flex items-center gap-1.5 text-corpo font-semibold px-3 py-2 whitespace-nowrap transition-colors ${
            ...
            {rows.map((r) => (
              <div key={r.id} className="px-4 py-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  {(r.sub || r.phone) && (
                    <p className="text-corpo text-tx-2 truncate mt-0.5">{[r.sub, r.phone].filter(Boolean).join(" · ")}</p>
                  )}
                </div>
