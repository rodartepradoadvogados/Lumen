...
export default async function MobileAssessoriaDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      <Link href="/m/assessoria" className="inline-flex items-center gap-1 text-corpo font-semibold text-tx-2">
            ...
            {assessoria.honorarios.map((h) => (
              <div key={h.id} className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-corpo tabular-nums text-tx-2">{formatCurrency(h.receivable.amount)} · vence {formatDate(h.receivable.dueDate)}</p>
                </div>
        ...
        acoes={
          ...
          {/* Auditoria apontou que esta tela só vinculava processo existente — nenhuma criação
            ...
            <Link
              ...
              className="flex items-center gap-1 text-corpo font-semibold text-marca-tx px-2.5 py-1 shrink-0 min-h-[44px]"
            >
