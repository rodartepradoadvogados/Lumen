...
export default async function MobileRelatoriosFinanceiro() {
  ...
  const [movimentos, overdueReceivables, allReceivables] = await Promise.all([
    ...
    // PARCIAL entra também — mesma regra da Central de Alertas (lib/alerts.ts): já teve parte
    // paga, mas ainda tem saldo em aberto vencido. O denominador logo abaixo (allReceivables) já
    // incluía PARCIAL; sem isto aqui, a taxa de inadimplência tinha numerador e denominador
          ...
          {financeMonthly.map((m) => (
            <div key={m.key} className="px-4 py-3 flex items-center justify-between gap-3">
              ...
              <div className="flex-1 text-right leading-tight">
                <p className="text-corpo tabular-nums text-concluido">+{formatCurrency(m.receita)}</p>
                <p className="text-corpo tabular-nums text-urgente">-{formatCurrency(m.despesa)}</p>
              </div>
              <span
...
function StatBlock({
  ...
  return (
    <Card className="p-4">
      <p className="text-corpo font-medium text-tx-2 uppercase tracking-wide">{label}</p>
      <p className={`text-2xl font-bold tabular-nums mt-1 ${toneClass}`}>{value}</p>
      {hint && <p className="text-corpo text-tx-2 mt-1">{hint}</p>}
    </Card>
  );
}
