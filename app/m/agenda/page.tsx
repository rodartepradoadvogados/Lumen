    ...
    <div className="p-4 space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="text-center min-w-0 flex-1">
          <p className="font-bold text-tx text-sm inicial-maiuscula truncate">{label}</p>
          {!isToday && (
    ...
    <Link href={href} className={`flex items-start gap-3 px-4 py-3.5 border-l-[3px] ${isPagar ? "border-l-atencao" : "border-l-concluido"}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap mb-0.5">
          <span className={`text-corpo font-semibold ${isPagar ? "text-atencao" : "text-concluido"}`}>{isPagar ? "Conta a Pagar" : "Conta a Receber"}</span>
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <p className="font-bold text-tx text-sm inicial-maiuscula text-center flex-1 truncate">
          {monthLabel}
        </p>
        <Link
      ...
      <Card className="p-3">
        <div className="grid grid-cols-7 gap-1.5">
          {WEEKDAY_LABELS.map((wd, i) => (
            <div key={i} className="text-center text-corpo font-semibold text-tx-2">
              {wd}
            </div>
          ))}
          {weekDays.map((d) => {
      ...
      <p className="text-center text-corpo text-tx-2">Toque em um dia para ver as atividades.</p>
    </div>
  );
}
