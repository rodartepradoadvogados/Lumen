...
export default async function AttendanceDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-4 md:inset-8 lg:inset-12 z-50 bg-cream-50 rounded-xl shadow-pop flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-3 border-b border-navy-800/8 shrink-0 bg-cream-50">
          <span className="text-xs font-semibold text-navy-800/50 uppercase tracking-wide">Atendimento</span>
          <Link
            href="/atendimento"
            className="text-navy-800/40 hover:text-navy-900 transition-colors"
            aria-label="Fechar"
            title="Fechar"
          >
            <X size={20} />
        ...
        <div className="overflow-y-auto flex-1 p-6">
          <div className="max-w-[900px] mx-auto animate-fade-in">
            ...
            <p className="text-sm text-navy-800/50">{a.subject}</p>
            <p className="text-xs italic text-slate-600 mt-1.5 mb-5">
            ...
            {!a.convertedCaseId && (
              <Card className="p-5 mb-5">
                <h4 className="text-sm font-semibold text-navy-900 mb-1">Transformar em Processo/Caso</h4>
            ...
            {showWhatsapp && (
              <Card className="p-5 mb-5">
                <div className="flex items-start justify-between mb-3 gap-2">
                  ...
                  {a.waPhone && <span className="text-xs text-navy-800/45 shrink-0">{a.waPhone}</span>}
                </div>
            ...
            <Card className="mb-5">
              <div className="flex items-start justify-between px-5 py-3 border-b border-navy-800/8 gap-2">
                <div>
