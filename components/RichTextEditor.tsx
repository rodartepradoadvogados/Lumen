...
// dependência nova (ver decisão no relatório da entrega do painel Anotações — o projeto não
// tinha Tiptap/Slate/Quill nem nada parecido; para o escopo pedido, negrito/itálico/sublinhado/
// lista com marcadores/lista numerada, isso cobre tudo sem adicionar peso ao bundle). execCommand
// é formalmente "deprecated" mas segue amplamente suportado nos navegadores usados pelo
// escritório; se algum dia precisar de mais que formatação básica (tabelas, imagens inline,
// colaboração), aí sim vale trazer uma lib. HTML salvo precisa passar por
// lib/richText.ts:sanitizeRichTextHtml no servidor antes de persistir (nunca confiar no HTML que
// chega do client).
  ...
  return (
    <div className=" border border-regua overflow-hidden bg-sf">
      ...
      <div
        ...
        className="rich-text-editor min-h-[110px] max-h-[240px] overflow-y-auto px-2.5 py-2 text-sm text-tx focus:outline-none empty:before:content-[attr(data-placeholder)] empty:before:text-tx-3 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:my-0.5"
      />
    </div>
  );
}
