    ...
    <div>
      ...
      <div className="mt-1 flex flex-wrap items-center gap-1.5">
        {selected.map((m) => (
          <span
            key={m}
            className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-gold-500/15 text-gold-700 dark:text-gold-400"
          >
            {m}
            <input type="hidden" name="materias" value={m} />
            <button type="button" onClick={() => remove(m)} className="hover:text-bordo-600 dark:hover:text-bordo-400">
              <X size={11} />
            </button>
          </span>
        ))}
        {disponiveis.length > 0 && (
          <select
            ...
            className="text-xs font-medium border border-dashed border-navy-800/20 dark:border-white/20 rounded-full px-2.5 py-1 bg-transparent text-navy-800/60 dark:text-cream-50/60"
          >
