...
import { FilePlus, Download, FileDown, Paperclip, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function GerarDocumentoButton({ caseId, attendanceId }: { caseId?: string; attendanceId?: string }) {
  ...
  return (
    <>
      ...
      {open && (
        // "medio": no caso comum (categoria + modelo) são só 2 campos; a seção de honorários do
        // Contrato pode crescer bastante, mas ainda cabe confortavelmente sem precisar de 80%.
        <ModalShell size="medio" title={generated ? "Documento gerado" : "Gerar Documento"} onClose={handleClose}>
      ...
      <style jsx global>{`
        ...
        .dark .gd-input { border-color: rgba(255,255,255,0.15); background: #0f1f3d; color: #fbfaf7; }
      `}</style>
    </>
  );
}
