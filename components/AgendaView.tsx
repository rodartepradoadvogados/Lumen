...
const typeMeta: Record<string, { dot: string; chip: string }> = {
  TAREFA: { dot: "bg-slate-400", chip: "bg-slate-100 text-slate-700" },
  EVENTO: { dot: "bg-blue-500", chip: "bg-blue-100 text-blue-700" },
  AUDIENCIA: { dot: "bg-gold-500", chip: "bg-gold-500/20 text-gold-800" },
  PERICIA: { dot: "bg-amber-500", chip: "bg-amber-100 text-amber-700" },
  PRAZO: { dot: "bg-red-500", chip: "bg-red-100 text-red-700" },
};
...
export default function AgendaView({
  ...
  const controls = (
    <div className="flex flex-wrap items-center gap-3 mb-4">
      <div className="flex gap-1 bg-white rounded-lg border border-navy-800/10 p-1">
        {[
          { key: "mes", label: "Mês" },
          ...
          <Link
            ...
            className={clsx(
              ...
              visao === v.key ? "bg-navy-900 text-white" : "text-navy-800/60 hover:bg-cream-100"
            )}
          >
            {v.label}
          </Link>
      ...
      <select
        ...
        className="text-xs font-medium border border-navy-800/12 rounded-lg px-2.5 py-2 bg-white"
      >
      ...
      <div className="flex items-center gap-3 flex-wrap ml-auto">
        <span className="text-[11px] font-semibold text-navy-800/40 uppercase tracking-wide">Cores por tipo:</span>
        {Object.entries(typeMeta).map(([k, m]) => (
          <span key={k} className="flex items-center gap-1.5 text-[11px] text-navy-800/60">
    ...
    <div className="bg-white rounded-xl border border-navy-800/8 shadow-card flex flex-col min-h-0">
      <div className="flex items-center justify-between px-5 py-4 border-b border-navy-800/8">
        ...
        <div className="flex items-center gap-1">
          ...
          <Link href={buildHref({ year: String(today.getFullYear()), month: String(today.getMonth()) })} className="text-xs font-semibold text-gold-700 px-2 py-1 rounded-lg hover:bg-gold-500/10">
            Hoje
          </Link>
          ...
          return (
            ...
            >
              ...
              <div className="mt-1 space-y-0.5 overflow-hidden">
                ...
                {dayTasks.length > 3 && <p className="text-[10px] text-navy-800/40 pl-1">+{dayTasks.length - 3} mais</p>}
              </div>
            </button>
          );
        })}
...
function WeekView({
  ...
  return (
    <div className="bg-white rounded-xl border border-navy-800/8 shadow-card flex flex-col min-h-0">
      <div className="flex items-center justify-between px-5 py-4 border-b border-navy-800/8">
        <h3 className="font-serif font-bold text-navy-900 text-lg">Semana de {label}</h3>
        <div className="flex items-center gap-1">
          <Link href={buildHref({ week: ymd(prevWeek) })} className="p-1.5 rounded-lg hover:bg-cream-100 text-navy-800">
            <ChevronLeft size={18} />
          </Link>
          <Link href={buildHref({ week: ymd(today) })} className="text-xs font-semibold text-gold-700 px-2 py-1 rounded-lg hover:bg-gold-500/10">
            Hoje
          </Link>
          <Link href={buildHref({ week: ymd(nextWeek) })} className="p-1.5 rounded-lg hover:bg-cream-100 text-navy-800">
            <ChevronRight size={18} />
          </Link>
        </div>
      </div>
        ...
        {days.map((date) => {
          ...
          return (
            <button
              ...
              onClick={() => setSelected(key)}
              className={clsx(
                "border-r border-navy-800/5 last:border-r-0 p-1.5 text-left flex flex-col overflow-hidden transition-colors",
                isSelected && "bg-gold-500/10"
              )}
            >
              <div className="flex flex-col items-center mb-1.5">
                <span className="text-[10px] font-semibold text-navy-800/40 uppercase">{WEEKDAYS[date.getDay()]}</span>
                <span
                  className={clsx(
...
function ListView({ tasksByDay }: { tasksByDay: Record<string, TaskData[]> }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-navy-800/8 shadow-card flex-1 min-h-0 overflow-y-auto scrollbar-thin">
      {days.length === 0 && <p className="text-center text-sm text-navy-800/35 py-16">Nada agendado nos próximos 30 dias</p>}
      {days.map((day) => {
        const items = tasksByDay[day].sort((a, b) => (a.dueTime || "").localeCompare(b.dueTime || ""));
        const date = new Date(day + "T00:00:00");
        return (
          <div key={day}>
            <div className="sticky top-0 bg-cream-50 px-5 py-2 border-y border-navy-800/8">
              <p className="text-xs font-semibold text-navy-900 capitalize">
                ...
                return (
                  <div key={t.id} className="px-5 py-3 flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className={clsx("text-sm font-medium text-navy-900 mt-1", done && "line-through text-navy-800/40")}>{t.title}</p>
                      {t.case && (
                        <Link href={`/processos/${t.case.id}`} className="text-xs text-gold-700 hover:underline block truncate">
                          {t.case.title}
                        </Link>
                      )}
                      {t.responsible && <p className="text-[11px] text-navy-800/40 mt-0.5">Responsável: {t.responsible.name}</p>}
                    </div>
                  </div>
                );
              })}
...
function DayPanel({
  ...
  return (
    <div className="bg-white rounded-xl border border-navy-800/8 shadow-card flex flex-col min-h-0">
      <div className="px-5 py-4 border-b border-navy-800/8 flex items-start justify-between gap-3">
        <div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin divide-y divide-navy-800/5">
        {selectedTasks.length === 0 && <p className="text-center text-sm text-navy-800/35 py-10">Nada agendado para este dia</p>}
        {selectedTasks.map((t) => {
          const done = t.status === "CONCLUIDO";
          return (
            <div key={t.id} className="px-5 py-3.5 flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {t.meetingType === "PRESENCIAL" && t.location && <p className="text-[11px] text-navy-800/50 mt-1">📍 {t.location}</p>}
