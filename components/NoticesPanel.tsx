        ...
        <div className="flex gap-2">
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full resize-none border border-navy-800/12 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500/40"
              onKeyDown={(e) => {
            ...
            {mentionQuery !== null && mentionCandidates.length > 0 && (
              <div className="absolute left-0 bottom-full mb-1 w-56 bg-white rounded-lg border border-navy-800/10 shadow-pop z-20 overflow-hidden max-h-48 overflow-y-auto scrollbar-thin">
                {mentionCandidates.map((u) => (
                  <button
                    key={u.id}
                    type="button"
                    onClick={() => selectMention(u.name)}
                    className="flex items-center w-full px-3 py-2 text-sm text-navy-900 hover:bg-cream-50 transition-colors text-left"
                  >
                    {u.name}
                  </button>
                ))}
