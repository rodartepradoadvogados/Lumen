  ...
  return (
    <>
      <div className="flex items-start justify-between gap-3">
        ...
        <Link href={`/m/assessoria/${assessoriaId}/pareceres/${parecer.id}/editar`} className="flex items-center gap-1 text-xs font-semibold text-tx-2 shrink-0 px-2 py-1">
          <Pencil size={13} /> Editar
        </Link>
      </div>
      <p className="text-xs text-tx-2 mt-0.5">{formatDate(parecer.date)}</p>
      ...
      <Card className="p-4 mt-3">
                ...
                <div key={d.id} className="flex items-center gap-2.5 py-2.5">
                  <a href={d.driveUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-tx-2 mt-0.5">{getDocumentTypeLabel(d.docType)} · {formatDate(d.date)}</p>
                    </div>
        ...
        {docDeleteError && <p className="text-[11px] text-urgente mt-1">{docDeleteError}</p>}
      ...
      <div className="mt-4 px-1">
        <button type="button" onClick={handleDeleteParecer} disabled={pending} className="flex items-center gap-1 text-xs font-semibold text-tx-2 disabled:opacity-50">
