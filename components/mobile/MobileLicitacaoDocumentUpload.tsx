...
export default function MobileLicitacaoDocumentUpload({
  ...
  if (!file) {
    return (
      <button
        ...
        className="w-full flex items-center justify-center gap-1.5 text-xs font-semibold text-acao border border-dashed border-regua hover:border-acao/40 py-2.5 transition-colors mt-2"
      >
  ...
  return (
    <div className="border border-regua bg-sf-apoio p-2.5 space-y-2 mt-2">
      ...
      />
      <DocumentTypeSelect value={docType} onChange={setDocType} className="w-full text-xs border border-regua bg-sf text-tx px-2.5 py-1.5" allowCreate />
      {showTaskPicker && (
        <select value={assignTaskId} onChange={(e) => setAssignTaskId(e.target.value)} className="w-full text-xs border border-regua bg-sf text-tx px-2.5 py-1.5">
