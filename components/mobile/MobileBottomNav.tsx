...
export default function MobileBottomNav({ unreadCount }: { unreadCount: number }) {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 h-16 bg-white border-t border-navy-800/10 flex z-40 shadow-[0_-2px_12px_rgba(15,31,61,0.06)]">
        ...
        return (
          ...
          >
            <span className="relative">
              <Icon size={22} className={active ? "text-gold-600" : "text-navy-800/40"} />
              {badge && unreadCount > 0 && (
            ...
            <span className={`text-[10px] font-medium leading-none ${active ? "text-navy-900" : "text-navy-800/40"}`}>
              {label}
            </span>
          </Link>
        );
