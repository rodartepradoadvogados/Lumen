  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      <input
        ...
        className="w-full rounded-[2px] pl-9 pr-3 py-3 border border-regua bg-sf text-sm text-tx placeholder:text-tx-3 shadow-card focus:outline-none focus:ring-2 focus:ring-marca-tx"
      />

      {showDropdown && (
        <div className="absolute left-0 right-0 top-full mt-2 rounded-[2px] bg-sf border border-regua shadow-pop z-50 overflow-hidden max-h-[60vh] overflow-y-auto scrollbar-thin">
            ...
            GROUP_ORDER.map((group) => {
              ...
              return (
                <div key={group} className="border-b border-regua last:border-0">
                  <p className="px-4 pt-2.5 pb-1 text-corpo font-semibold text-tx-2 uppercase tracking-wide">
                    {group}
                  </p>
                  {groupItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {item.subtitulo && (
                          <span className="text-corpo text-tx-2 truncate w-full">
                            {item.subtitulo}
                          </span>
                        )}
                      </button>
