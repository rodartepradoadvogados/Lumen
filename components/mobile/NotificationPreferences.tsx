...
export default function NotificationPreferences() {
  ...
  if (!supported) {
    return (
      <p className="px-4 py-3 text-corpo text-tx-2">
  ...
  return (
    <div>
      <div className="px-4 py-3.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-corpo text-tx-2">{subscribed ? "Ativadas" : "Desativadas"}</p>
          </div>
        </div>
        <button
          type="button"
          ...
          className={`text-corpo font-semibold px-3 py-1.5 transition-colors disabled:opacity-50inline-flex items-center min-h-[44px]${
            subscribed
      ...
      {error && <p role="alert" className="px-4 pb-2 text-corpo font-semibold text-urgente">{error}</p>}
