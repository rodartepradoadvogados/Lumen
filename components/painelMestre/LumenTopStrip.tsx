...
export default function LumenTopStrip({ memberName }: { memberName: string }) {
  return (
    <header className="h-14 shrink-0 bg-navy-900 dark:bg-navy-900 border-b border-white/10 flex items-center justify-between px-5">
      <Link
        href="/painel"
        className="text-xs font-semibold text-navy-800/60 dark:text-cream-50/60 hover:text-navy-900 dark:hover:text-cream-50"
      >
        ← Voltar ao escritório
      </Link>
      <div className="flex items-center gap-2">
        <span className="text-sm text-navy-900 dark:text-cream-50">{memberName}</span>
