...
export default function EditSupplierModal({ supplier }: { supplier: SupplierData }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} data-tip="Editar fornecedor" className="p-1.5 rounded-lg text-navy-800/30 hover:text-navy-900 hover:bg-cream-100 transition-colors">
        <Pencil size={14} />
      </button>
      {open && (
        <div className="fixed inset-0 z-50 bg-navy-950/40 flex items-center justify-center p-4" onClick={() => setOpen(false)}>
          <div className="bg-white rounded-xl shadow-pop w-full max-w-md max-h-[90vh] overflow-y-auto scrollbar-thin" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-navy-800/8">
              <h3 className="font-serif font-bold text-navy-900">Editar Fornecedor</h3>
            ...
            >
              ...
              <div>
                ...
                <textarea name="notes" rows={2} defaultValue={supplier.notes || ""} className="cl-input" />
              </div>
