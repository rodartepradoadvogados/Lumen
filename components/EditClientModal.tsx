...
export default function EditClientModal({ client }: { client: ClientData }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} data-tip="Editar cliente" className="p-1.5 text-tx-3 hover:text-tx hover:bg-sf-apoio transition-colors rounded-md">
        <Pencil size={14} />
      </button>
      {open && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 items-start">
                ...
                <div className="space-y-3">
                  <div className="border border-regua p-3 space-y-3">
                    <p className="text-etiqueta font-semibold text-tx-2 uppercase tracking-wide">Dados para geração de documentos</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
